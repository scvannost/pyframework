...
class AbstractDatabase(abc.ABC):
	...
	*db_query(q, limit, kwargs)
	...
	contains(), eq(), ge(), gt(), like(), logical_and(), logical_or(), lt(), le(), ne()
		route to self.translator.[func]()
	"""
	def __init__(self, location : str, user : str, password : str, name : str, *args, Translator : Type['AbstractTranslator'], **kwargs) -> None:
...
class AbstractSqlDatabase(AbstractDatabase):
	...
	*db_query(q, limit, kwargs)
	...
	def add_column(self, table: Union[str, Table], new: Union[str, Column], after: Union[str, Column] = None) -> Table:
		"""
	...
	def alter_column(self, table: Union[str, Table], old: Union[str, Column], new: Union[str, Column]) -> Table:
		"""
	...
	def drop_table(self, table: Union[str, Table], temp: bool = False) -> 'AbstractSqlDatabase':
		"""
	...
	def join(self, one : Union[str, Table], two : Union[str, Table], on : Union[str, Column], direction : str, *, alias : str = "") -> Table:
		return self.translator.join(one, two, on, direction, alias=alias)


	def make_table(self, table: Union[str, Table], columns: Union[str, Column, Mapping[str, str], Iterable[Union[str, Column]]], temp: bool = False, clobber: bool = False) -> Table:
		"""
	...
	def query(self, method: str, table: Union[str, Table], fields: Union[None, Mapping[Union[str, Column], Any], Iterable[str], Iterable[Column], str, Column, Table] = None, *, where : Union[str, Column] = None, limit : int = None, groupby : Union[str, Column] = None, orderby : Union[str, Column] = None, **kwargs) -> Any:
		"""
		...
		*
		...
		"""
		self.validate_and_raise(method, table, fields, where=where, limit=limit, groupby=groupby, orderby=orderby, **kwargs)

		# call db_query to get results
		results = self.db_query(self.translate(method, table, fields, where=where, limit=limit, groupby=groupby, orderby=orderby, **kwargs), **kwargs)

		return self.translator.interpret(results, method, table, fields, where=where, limit=limit, groupby=groupby, orderby=orderby, **kwargs)
...
class AbstractSqlTranslator(AbstractTranslator):
	...
	*groupby(), *join(), *limit(), *orderby(), *where()
	...
	def div(self, a : Any, b : Any) -> Any:
		return Column((a,b), f'({a!r} / {b!r})', 'operation')
	def eq(self, a : Any, b : Any) -> Any:
		if not a: a = self.null
		if not b: b = self.null

...
class MysqldbDatabase(AbstractMySqlDatabase):
	...
	**kwargs
	...
	def connect(self) -> None:
		...
		self._db = MySQLdb.connect(self._location, self._user, self._password, self._database, use_unicode=True, charset='utf8mb4')
		...
		if tables:
			...
			for i in tables:
				...
				setattr(self, i, Table(self, i, [Column.from_defintion(c) for c in columns]))

			self._tables = [getattr(self, i) for i in tables]

		return self

	def db_query(self, q: str, **kwargs) -> Union[Dict[str, Any], List[Tuple[Any]], List[Dict[str, Any]]]:
		"""
...
class MysqldbTranslator(AbstractMySqlTranslator):
	...
	def interpret(self, results : Union[Dict[str, Any], List[Tuple[Any]], List[Dict[str, Any]]], method: str, table: Union[str, Table], *args, **kwargs) -> Union[int, Iterable[Union[Any, Mapping[str, Any]]]]:
		"""
		...
		if method == 'count':
			...
			if 'how' in kwargs and kwargs["how"] == 0:
				...
				# ( {COLUMNS.key : value} )
				results = [f"`{r['COLUMNS.Field']}` {r['COLUMNS.Type']}" for r in results]
			else:
				# ( {key : value} )

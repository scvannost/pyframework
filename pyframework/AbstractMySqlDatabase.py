...
from .AbstractDatabase import AbstractSqlDatabase, AbstractSqlTranslator, AbstractTranslator
...
class AbstractMySqlDatabase(AbstractSqlDatabase):
	...
	*db_query(q, limit, kwargs)
	...
	contains(), eq(), ge(), gt(), like(), logical_and(), logical_or(), lt(), le(), ne()
		route to self.translator.[func]()
	"""
	def __init__(self, *args, Translator : Type[AbstractTranslator] = None, **kwargs) -> None:
		if Translator is None:
			Translator = AbstractMySqlTranslator
		super().__init__(*args, Translator=Translator, **kwargs)


	def add_fk(self, name: str, table: str, field: str, ref: str) -> 'AbstractMySqlDatabase':
		"""
...
class AbstractMySqlTranslator(AbstractSqlTranslator):
	...
	*interpret(results, *args, **kwargs)
	...
	@abc.abstractmethod
	def interpret(self, results : Any,  method: str, *args, **kwargs) -> Union[int, Iterable[Union[Any, Mapping[str, Any]]]]:
		"""
	...
	def validate_and_raise(self, method: str, table: Union[str, Table], fields: Union[None, Mapping[Union[str, Column], Any], Iterable[str], Iterable[Column], str, Column, Table] = None, *, where : Union[str, Column] = None, limit : int = None, groupby : Union[str, Column] = None, orderby : Union[str, Column] = None, **kwargs):
		"""
		...
		*
		...
		if 'show' not in method or 'table' not in table: # don't check for @method='show table'
			# try to interpret table and if it belongs to db
			if not isinstance(table, (str, Table)):
				raise TypeError('Database.query@table must be a Table or str in this database (except if @method="create table")')
			if ("add" in method or "create" in method) and "table" in method and self.db.get_table(table) is not None:
				raise ValueError('Database.query@table cannot be a table in self.tables for @method="create table"')
			elif ("create" not in method and "add" not in method) or "table" not in method:
				table = self.db.get_table(table) # need for validation
				if table is None:
					raise ValueError('Database.query@table must be a table in self.tables for @method!="create table"')	

		# BEGIN @method / @table / @fields validation
		if method == 'insert':
			if not isinstance(fields, Mapping):
				raise TypeError('Database.query@fields must be Mapping[col, value] for @method=insert.')
			for k in fields.keys():
				if not isinstance(k, (str, Column)):
					raise TypeError('Database.query@fields must be Mapping[col, value] for @method=insert, where col is a str or Column')
				if table.get_column(k) is None:
					raise ValueError('Database.query@fields may only contains keys in @table.columns for @method=insert')

			for c in table.columns:
				if c.reqd and c.name not in fields:
					raise ValueError('Database.query@fields must be passed all required fields in @table for @method=insert')

		elif method == 'update':
			if not isinstance(fields, Mapping):
				raise TypeError('Database.query@fields must be a Mapping for @method=update.')
			for k in fields.keys():
				if not isinstance(k, (str, Column)):
					raise TypeError('Database.query@fields must be Mapping[col, value] for @method=update, where col is a str or Column')
				elif table.get_column(k) is None:
					raise ValueError('Database.query@fields may only contains keys in @table.columns for @method=update')			
			...
			if not isinstance(fields, str):
				raise TypeError('Database.query@fields must be a str for @method="rename table"')
			elif self.db.get_table(fields) is not None:
				raise ValueError('Database.query@fields must not be an existing table for @method="rename table"')

		elif 'drop' in method and 'column' in method:
			if not isinstance(fields, (str, Column)):
				raise TypeError('Database.query@fields must be a column in @table for @method="drop column"')
			if table.get_column(fields) is None:
				raise ValueError('Database.query@fields must be a column in @table for @method="drop column"')


		elif ('create' in method or 'add' in method) and 'table' in method:
			if isinstance(table, str):
				if isinstance(fields, str):
					fields = [f.strip() for f in fields.split(",")] # needed for validation
					table = self.escape_string(table) # just in case

					used_names = []
					for f in fields:
						if not self.is_valid_column(Table("", table, []), f):
							raise ValueError('Database.query@fields must be a str or Iterable(col) for @method="create table" unless @table is a Table')
						elif isinstance(f, str) and f.rsplit(' ',1)[0] in used_names:
							raise ValueError('Database.query@fields may not contain duplicate column names for @method="create table" unless @table is a Table')
						elif isinstance(f, Column) and f.name in used_names:
							raise ValueError('Database.query@fields may not contain duplicate column names for @method="create table" unless @table is a Table')
						else:
							used_names.append(f.name if isinstance(f, Column) else f.rsplit(' ', 1)[0])
					...
					for f in fields:
						if not self.is_valid_column(Table("", table, []), f):
							...
							raise ValueError('Database.query@fields may not contain duplicate column names for @method="create table"')
						elif isinstance(f, Column) and f.name in used_names:
							raise ValueError('Database.query@fields may not contain duplicate column names for @method="create table"')
						else:
							used_names.append(f.name if isinstance(f, Column) else f.rsplit(' ', 1)[0])
					...
					raise TypeError('Database.query@fields must be a str or Iterable(col) for @method="create table" unless @table is a Table')

			elif fields is not None: # table is a Table
				raise ValueError('Database.query@fields must be None for @method="create table" and @table is a Table')

		elif ('add' in method or 'create' in method) and 'column' in method:
			if not self.is_valid_column(table, fields):
				raise ValueError('Database.query@fields must be a valid column definition for the table you are adding it to')

			if 'after' in kwargs:
				if not self.db.get_column(table, kwargs['after']) and not (isinstance(kwargs['after'], str) and kwargs['after'] == "first"):
					raise ValueError('Database.query@after must be a column in the table or "first" if given')
			...
			if isinstance(fields, Mapping):
				if len(fields) != 1:
					raise ValueError('Database.query@fields must have only a single key for @method="alter column" if @fields is a dict')
				elif 'to' in kwargs and kwargs['to'] is not None:
					raise ValueError('Database.query@to should not be specified for @method="alter column" if @fields is a dict')

				kwargs['to'] = list(fields.values())[0] # needed for validation
				fields = list(fields.keys())[0] # also this

			if not table.get_column(fields):
				raise ValueError('Database.query@fields must be a column in @table for @method="alter column"')

			if 'to' not in kwargs:
				raise ValueError('Database.query@to must be provided for @method="alter column"')
			elif not self.is_valid_column(table, kwargs['to']):
				raise ValueError('Database.query@to must be a valid column definition for the table')
			...
			if not isinstance(fields, str):
				for n in range(len(fields)):
					if table.get_column(fields[n]) is None:
						raise TypeError('Database.query@fields must be \'all\' or Iterable(col).')

			elif fields != 'all' and table.get_column(fields) is None:
				raise ValueError('Database.query@fields must be \'all\' or Iterable(col).')

		elif method == 'delete':
			fields, groupby, orderby, limit = None, None, None, None,
			...
			raise ValueError(f"See documentation for allowed values of Database.query@method; {method!s} not allowed")
		# END @method / @table / @fields validation
		
		if where and not (isinstance(where, Column) and where.dtype == 'comparison'):
			raise TypeError('Database.query@where must be a Column with dtype "comparison"')
		...
		for name,check in {"groupby":groupby, "orderby":orderby}.items():
			if check:
				...
				elif check not in table.columns + table.column_names + [table.name + "." + c for c in table.column_names]:
					raise ValueError(f"Database.query@{name} must be a column in @table")


	def translate(self, method: str, table: Union[str, Table], fields: Union[None, Mapping[Union[str, Column], Any], Iterable[Union[str, Column]], str, Column, Table] = None, *, where : Union[str, Column] = None, limit : int = None, groupby : Union[str, Column] = None, orderby : Union[str, Column] = None, **kwargs):
		"""
		...
		*
		...
		"""
		if (("create" not in method and "add" not in method) or "table" not in method) and ('show' not in method or 'tables' not in table):
		...
		if method == 'insert':
			...
			if any([i in method for i in ['if not exists', 'ifnot exists', 'if notexists', 'ifnotexists']]) or "clobber" not in kwargs:
		...
		if method in ['select','distinct','count']:
			...
			sql = f"update {table!r} set " + ', '.join( [f"{f!r} = {self.escape_string(v)}" for f,v in fields.items()] )

		elif method == 'describe':
			sql = f"describe {table!r}"

		elif 'drop' in method and 'table' in method:
			sql = 'drop ' + ('temporary ' if kwargs["temp"] else '') + f"table {table!r} if exists"

		elif 'drop' in method and 'column' in method:
			sql = f"alter table {table!r} drop column {fields}"

		elif ('create' in method or 'add' in method) and 'table' in method:
			sql = 'create ' + ('temporary ' if kwargs["temp"] else '') + f"table {table!r}"
			sql += (' if not exists' if not kwargs["clobber"] else '') + ' ('
			sql += ', '.join([f"{f.name!r} {f.dtype!s}" if isinstance(f, Column) else str(f) for f in fields])
			sql += ')'
			...
			sql += f"{fields.name!r} {fields.dtype!s}" if isinstance(fields, Column) else str(fields)
			if 'after' in kwargs:
				if kwargs['after'].lower() == 'first': sql += ' first'
				else: sql += f" after {kwargs['after']!r}"

		elif 'alter' in method and 'column' in method:
			sql = f"alter table {table!r} change column {fields!r} "
			sql += f"{kwargs['to'].name!r} {kwargs['to'].dtype!s}" if isinstance(kwargs['to'], Column) else str(kwargs['to'])
		elif 'show' in method and 'tables' in table:
			sql = "show tables"
	...
	def join(self, one : Union[str, Table], two : Union[str, Table], on : Union[str, Column], direction : str, *, alias : str = "") -> str:
		return f"{one.name} {direction.lower()} join {two.name} " + (f"as {alias!r} " if alias else '') + f"on {on!r}"

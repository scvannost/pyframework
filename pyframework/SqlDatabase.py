...
class SqlDatabse(AbstractDatabase):
    def query(
        ...
        method: str,
        tbl: str,
        fields: Union[None, Dict[str, Any], List[str], str] = None,
        extra: str = None,
        **kwargs
    ):
        """
        ...
        if not method in ["distinct", "select", "insert", "delete", "update", "count"]:
            raise ValueError(
                "Database.query can only select, insert, delete, count, or update."
            )
        ...
        if not method in ["count", "delete"]:
            if method in ["insert", "update"]:
                if not type(fields) is dict:
                    raise TypeError(
                        "Database.query@fields must be a dict for insert. 'all' is not accepted."
                    )
            elif type(fields) is list:
                for i in fields:
                    if not type(i) is str:
                        raise TypeError(
                            "Database.query@fields must be 'all' or a list of str."
                        )
            else:

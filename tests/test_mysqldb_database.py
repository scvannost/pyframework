...
from pyframework import AbstractMySqlDatabase, MysqldbDatabase, MysqldbTranslator, Column, Table
import MySQLdb
...
def test_database_connect(mocker: MockerFixture):
	...
	MySQLdb.connect.assert_has_calls([
		mocker.call("loc", "user", "password", "database", use_unicode=True, charset="utf8mb4")
	])
...
def test_database_db_query(mocker: MockerFixture):
	...
	db._db.store_result.return_value.fetch_row.return_value = [{"a":bytes("foo", "utf8")}]
...
def test_translator(mocker: MockerFixture):
	...
	MySQLdb.escape_string.assert_has_calls([
		mocker.call("foo")
	])

	assert db.translator.interpret(results=None, method="foo", table="bar") is None
	assert db.translator.interpret(results=("foo", "bar"), method="select", table="baz") == ["foo", "bar"]
	assert db.translator.interpret(results=("foo", "bar"), method="distinct", table="baz") == ["foo", "bar"]
	assert db.translator.interpret(results=[["foo"]], method="count", table="bar", how=0) == "foo"
	assert db.translator.interpret(results=[{"a":"foo"}], method="count", table="bar") == "foo"
	assert db.translator.interpret(results=[("foo", "bar")], method="describe", table="baz", how=0) == ["`foo` bar"]
	assert db.translator.interpret(results=[{"COLUMNS.Field":"foo", "COLUMNS.Type":"bar"}], method="describe", table="baz", how=2) == ["`foo` bar"]
	assert db.translator.interpret(results=[{"Field":"foo", "COLUMNS.Type":"bar"}], method="describe", table="baz") == ["`foo` bar"]
	assert db.translator.interpret(results=[("foo",), ("bar",)], method="show", table="tables", how=0) == ["foo", "bar"]
	assert db.translator.interpret(results=[{"foo":"bar"}], method="show", table="tables") == ["bar"]

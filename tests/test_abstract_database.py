...
def mockdb(mocker : MockerFixture) -> MockMySqlDB:
	...
	mocker.patch.object(db, "_tables",
		[
			Table(db, "foo", []),
			Table(db, "bar", []),
		])
...
def test_passing_to_query(mockdb : MockMySqlDB, mocker : MockerFixture):
	...
	query.assert_has_calls([
			mocker.call("create table", "table", ["field1 int", "field2 int"],
						temp=False, clobber=False),
	...
	query.assert_has_calls([
			mocker.call("rename", "bar", "new"),
			mocker.call("describe", "new")
		])
	tbl = mockdb.tables[-1]
	assert tbl.name == 'new'
	assert tbl.db == mockdb
	...
	query.assert_has_calls([
			mocker.call("drop column", "table", "field"),
			mocker.call("describe", "table")
		])
	query.reset_mock()
...
def test_sql_generators(mockdb : MockMySqlDB):
	...
	assert mockdb.translator.join(mockdb.foo, mockdb.bar, "baz", "inner") == "foo inner join bar on 'baz'"
	assert mockdb.translator.join(mockdb.foo, mockdb.bar, "baz", "inner", alias="table") == "foo inner join bar as 'table' on 'baz'"
...
def test_query_validation(mockdb : MockMySqlDB, mocker : MockerFixture):
	...
	with pytest.raises(ValueError, match=r"may not contain duplicate column names"):
		mockdb.query("create table", "table", ["index int", Column("", name="index", dtype="float")])
	...
	mocker.patch.object(mockdb.foo, "_columns", [Table("","bar",[])])
	with pytest.raises(TypeError, match=r"@where must be a Column with dtype \"comparison\""):
...
def test_query_translate(mockdb : MockMySqlDB, mocker : MockerFixture):
	...
	assert mockdb.query('create table', tbl) == "create table 'table' if not exists ('column' int);"
	mockdb._tables.append(tbl)
	tbl._db = mockdb
	assert mockdb.query('drop table', tbl) == "drop table 'table' if exists;"
	mockdb._tables = []

	assert mockdb.query('create table', 'table', 'column int') == "create table 'table' if not exists (column int);"
	mockdb._tables.append(tbl)
	assert mockdb.query('drop table', 'table') == "drop table 'table' if exists;"
	mockdb._tables = []

	tbl._columns.append(Column(tbl, 'new_column', 'text'))
	assert mockdb.query('create table', 'table', 'column int, new_column text') == "create table 'table' if not exists (column int, new_column text);"
	assert mockdb.query('create table', 'table', ['column int', 'new_column text']) == "create table 'table' if not exists (column int, new_column text);"
	mockdb._tables.append(tbl)
	assert mockdb.query('show', 'tables') == "show tables;"
	assert mockdb.query('describe', 'table') == "describe 'table';"

	assert mockdb.query('insert', 'table', {'column':'value1', 'new_column':'value2'}) == "insert into 'table' ('column', 'new_column') values (value1, value2);"
	assert mockdb.query('update', 'table', {'column':'new_value'}) == "update 'table' set 'column' = new_value;"

	assert mockdb.query('rename table', 'table', 'new_name') == "alter table 'table' rename 'new_name';"
	mockdb._tables[0]._name = 'new_name'
	assert mockdb.query('rename table', 'new_name', to='table') == "alter table 'new_name' rename 'table';"
	mockdb._tables[0]._name = 'table'
	
	mockdb.tables[0]._columns = []
	assert mockdb.query('add column', 'table', 'column int') == "alter table 'table' add column column int;"
	mockdb.tables[0]._columns.append(Column(tbl, 'column', 'int'))
	col = Column(mockdb.get_table('table'), 'new_column', 'blob')
	assert mockdb.query('add column', 'table', col) == "alter table 'table' add column 'new_column' blob;"
	mockdb.tables[0]._columns.append(col)

	column, new_column = mockdb.tables[0]._columns
	assert mockdb.query('select', 'table', 'all', groupby=column) == "select * from 'table' group by table.column;"
	assert mockdb.query('select', 'table', 'all', orderby=new_column) == "select * from 'table' order by table.new_column;"
	assert mockdb.query('select', 'table', 'all', where=(column < 0)) == "select * from 'table' where (table.column < 0);"
	...
	assert mockdb.query('select', tbl.join(tbl2, on=(column==column2)), 'all') == "select * from table inner join new_table on (table.column = new_table.column);"


	col = Column(mockdb.get_table('table'), 'new_altered', 'date')
	assert mockdb.query('alter column', 'table', 'column', to='altered text') == "alter table 'table' change column 'column' altered text;"
	assert mockdb.query('alter column', 'table', 'new_column', to=col) == "alter table 'table' change column 'new_column' 'new_altered' date;"
	...
	assert mockdb.query('add temporary table', 'table', 'definition int') == "create temporary table 'table' if not exists (definition int);"
	mockdb._tables.append(tbl)
	assert mockdb.query('drop table temporary', 'table') == "drop temporary table 'table' if exists;"
	mockdb._tables = []
	
	assert mockdb.query('add temporary table if not exists', 'table', 'definition int') == "create temporary table 'table' if not exists (definition int);"
	mockdb._tables.append(tbl)
	assert mockdb.query('drop table', 'table', temp=True) == "drop temporary table 'table' if exists;"
	mockdb._tables = []
